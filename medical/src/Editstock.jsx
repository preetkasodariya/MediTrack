...
export function Editstock() {
    ...
    function updatestock(e) {
        ...
        axios.post("http://localhost/MediTrack/api/updatestock.php",formdata)
        .then((response)=>{
    ...
    function getsinglestock([id]){
        var formdata = new FormData();
        formdata.append("id",id);

        //API
        axios.post("http://localhost/MediTrack/api/getsinglestock.php",formdata)
            .then((response) => {
                if (response.status==200) {
                    var json = response.data;
                    setRid(json.Retailer_id);
                    setMid(json.medicine_id);
                    Quantity.current.value = json.quantity;
                    setPayment(json.ispay === "1" ? "yes" : "no");
                ...
                <div className="container">
                    <div className="card" >
                        ...
                        <div className="card-body">
                            ...
                            <form method="POST" onSubmit={updatestock}>
                                <div className="mb-3">
                                    <label className="form-label">Retailer id</label>
                                    <select ref={Retailer_id} className="form-control" id="rid" aria-describedby="emailHelp">
                                        <option>Please select Retailer</option>
                                        {
                                            retdata && retdata.map((row)=>{
                                                if(rid == row.Retailer_id)
                                                    {
                                                        return (<>
                                                            <option selected value={row.Retailer_id}>{row.name}</option>
                                                        </>);
                                                    }
                                                    else
                                                    {
                                ...
                                <div className="mb-3">
                                    ...
                                    <select ref={Medicine_id} className="form-control" id="mid" aria-describedby="emailHelp">
                                        ...
                                        {
                                            data && data.map((row)=>{
                                                if(mid == row.medicine_id)
                                                    {
                                                        return (<>
                                                            <option selected value={row.medicine_id}>{row.medicine_name}</option>
                                                        </>);
                                                    }
                                                    else
                                                    {
